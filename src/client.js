// DSH 桌面宠物 —— Client 半（宠物 UI / 动画 / 音效 / 交互）
//
// 这是一个 Cordis 动态插件的 Client 半「function body」。
// 它注册到 `shell.overlay` 槽位，在右下角渲染桌面宠物，
// 通过 CSS 动画与程序化合成音效响应 Agent 工作状态。
//
// 特性：
//   - 三只宠物：史蒂夫（含钻石甲皮肤）、苦力怕、奥特曼（初代/赛文/泰罗/迪迦/泽塔）
//   - 三态动作：待机浮动 / 工作动画 / 完成专属必杀 + 音效
//   - 交互：左键切换宠物、右键打开菜单、拖拽移动
//
// 作为动态插件使用时，直接取函数体内 `return { ... }` 的部分（见 README「如何安装」）。

'use strict'

module.exports = function createClientHalf() {
  return {
    inject: ['timer'],
    apply(ctx) {
      const SR = 16000
          ...
          const dispose = ctx.interval(function () {
            host.call('get-status').then(function (s) {
              ...
              if (s.phase === 'completed') {
                ...
                ctx.timeout(function () { setBeamOn(true) }, 1000)
                ctx.timeout(function () { setPhase(function (p) { return p === 'completed' ? 'idle' : p }); setBeamOn(false) }, 2600)
              } else {
                setPhase(s.phase === 'working' ? 'working' : 'idle')
              }
            }).catch(function () {})
          }, 500)
          return dispose
